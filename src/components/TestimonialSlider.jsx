      ...
      <div className="p-7 sm:p-10">
        ...
        <div className="flex items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} className="h-4 w-4 fill-accent text-[#C8A951]" />
            ))}
          </div>
          <div className="flex items-center gap-2">
            <button
              className="h-10 w-10 rounded-xl border border-black/10 bg-background hover:bg-[#08151F]/5 hover:border-primary/20 transition-colors grid place-items-center"
              onClick={() => setIdx((v) => (v - 1 + items.length) % items.length)}
              aria-label="Previous testimonial"
            >
              <ChevronLeft className="h-4 w-4 text-[#0B1C2C]" />
            </button>
            <button
              className="h-10 w-10 rounded-xl border border-black/10 bg-background hover:bg-[#08151F]/5 hover:border-primary/20 transition-colors grid place-items-center"
            ...
            >
              <ChevronRight className="h-4 w-4 text-[#0B1C2C]" />
            </button>
          </div>
        </div>
        ...
        <div className="min-h-[140px]">
          <AnimatePresence mode="wait">
            ...
            >
              <Quote className="h-7 w-7 text-[#C8A951]/40 mb-3" />
              ...
              <div className="mt-7 flex flex-wrap items-center justify-between gap-3 pt-5 border-t border-black/6">
                <div>
                  <div className="font-bold text-[#0B1C2C]">{current.name}</div>
                  <div className="text-sm text-[#4B5563]/60 mt-0.5">{current.context}</div>
                </div>
          ...
          {items.map((_, i) => (
            <button
              ...
              className={[
                ...
                i === idx ? 'w-8 bg-[#C8A951]' : 'w-2 bg-[#08151F]/15 hover:bg-[#08151F]/25',
              ].join(' ')}
